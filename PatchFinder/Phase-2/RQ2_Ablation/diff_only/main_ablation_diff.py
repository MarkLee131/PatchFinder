...
import model.ablation.diff_only.configs_ablation_diff as configs_ablation
...
if __name__ == '__main__':
    ...
    configs_ablation.get_singapore_time()
    logging.info('1/4: start to prepare the dataset.')

    train_data = CVEDataset(configs_ablation.train_file)
    valid_data = CVEDataset(configs_ablation.valid_file)
    test_data = CVEDataset(configs_ablation.test_file)
    ...
    wandb_logger = WandbLogger(name='train_ablation', project='PatchFinder_diff')
    ...
    CHECK_POINTS_PATH = "/mnt/local/Baselines_Bugs/PatchFinder/model/output_ablation_diff/Checkpoints"

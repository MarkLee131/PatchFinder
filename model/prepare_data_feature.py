...
def prepare_data():
    ...
    FIELD = data.Field(tokenize = desc_tokenize,
                    include_lengths=True,batch_first=True)
    ...
    if not configs.save_dataset:
        fields = [("diff", DIFF),("Msg", MSG),('Field',FIELD),("Cat", CAT),("Rule",RULE),("Priority",PRIORITY),("Rank",RANK),('FieldWarning',FIELDSWARNING),("Label",LABEL)]
        ...
        torch.save(DIFF, os.path.join(configs.save_path,'field_code.pth'))
        ...
        DIFF=torch.load(os.path.join(configs.save_path,'field_code.pth'))
        ...
        MAX_VOCAB_SIZE = 100000
        DIFF.build_vocab(train_data.diff,train_data.Field, max_size = MAX_VOCAB_SIZE)
        MSG.build_vocab(train_data, max_size = MAX_VOCAB_SIZE)
        LABEL.build_vocab(train_data)
        FIELDSWARNING.build_vocab(train_data, max_size = MAX_VOCAB_SIZE)
        FIELD.build_vocab(train_data.diff,train_data.Field, max_size = MAX_VOCAB_SIZE)
        # FIELD.build_vocab(train_data, max_size = MAX_VOCAB_SIZE)
